<template>
  ...
  <div class="flex flex-col h-screen bg-white dark:bg-gray-900 overflow-hidden">
    ...
    <div class="flex-shrink-0 p-4 bg-white dark:bg-gray-900 z-10">
      <div
            ...
            <div class="grid grid-cols-2 md:flex md:items-center gap-3 md:gap-0 w-full md:w-auto">
              <div class="flex items-center h-9 w-full md:w-auto min-w-0">
                ...
                <Select v-if="extensionStore.availableExtensions.length > 1" v-model="selectedExtensionValue"
                  ...
                  @change="handleExtensionChange" :loading="extensionStore.isLoading" checkmark
                  :highlightOnSelect="false" placeholder="Ext"
                  class="custom-extension-select flex items-center w-full md:w-auto" :pt="{
                  ...
                  <template #value="slotProps">
                    <div v-if="slotProps.value" class="flex items-center space-x-2 overflow-hidden">
                      ...
                      <span class="font-semibold truncate">{{extensionStore.availableExtensions.find(e =>
                        e.extension_uuid === slotProps.value)?.extension}}</span>
                    </div>
                ...
                <span v-else
                  class="text-sm font-medium dark:text-white flex items-center space-x-2 h-full w-full md:w-auto">
              ...
              <div v-if="agentName" class="flex items-center h-9 w-full md:w-auto min-w-0">
                <span class="text-sm text-gray-500 dark:text-gray-400 hidden md:inline mx-2">•</span>
                <div
                  class="flex items-center justify-center space-x-2 px-3 h-full w-full md:w-auto bg-blue-50 dark:bg-blue-900/20 rounded-md border border-blue-200 dark:border-blue-800 overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto pb-28 px-4 md:px-6 scroll-smooth">
      <router-view />
    </main>
...
const handleExtensionChange = async (event: { value: string }) => {
  ...
  if (extension) {
    ...
    if (success) {
      toast.add({ severity: 'success', summary: 'Switched Extension', detail: `Switched to ${extension.extension}`, life: 3000 })
    } else {
      toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to switch extension', life: 3000 })
    }
  }
}
